...
def get_args_parser(parser):
    ...
    parser.add_argument('--fix_update_iter', default=1, type=float)
    parser.add_argument('--threshold', default=0.3, type=float)
    parser.add_argument('--threshold_s', default=0.3, type=float)
    ...
    parser.add_argument('--alpha_aema', default=0.998, type=float)
    # mode settings
...
def teaching(model_stu, device):
    ...
    thresholds_s = [args.threshold_s] * args.num_classes
    # Record the best mAP
    ap50_best = -1.0
    ...
    or_model_stu = copy.deepcopy(model_tch)

    fig_dir = os.path.join('figs', str(datetime.datetime.now()))
    if is_main_process():
