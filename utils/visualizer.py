...
class COCOVisualizer():
    ...
    def visualize(self, img, tgt, caption=None, dpi=120, savedir=None, show_in_console=True):
        ...
        if savedir is not None:
            if caption is None:
                savename = '{}/{}-{}.jpg'.format(savedir, str(datetime.datetime.now()).replace(' ', '-'), tgt['image_id'])
            else:
                savename = '{}/{}-{}-{}.jpg'.format(savedir, caption, str(datetime.datetime.now()).replace(' ', '-'), tgt['image_id'])
    ...
    def addtgt(self, tgt):
        ...
        # if 'scores' in tgt:
        #     assert len(tgt['scores']) == numbox, f"{len(tgt['scores'])} = {numbox}, "
        #     for idx, score in enumerate(tgt['scores']):
        #         _string = str(score.item())[:4]
        #         bbox_x, bbox_y, bbox_w, bbox_h = boxes[idx]
        #         # ax.text(bbox_x, bbox_y, _string, color='black', bbox={'facecolor': 'yellow', 'alpha': 1.0, 'pad': 1})
        #         ax.text(bbox_x, bbox_y, _string, color='black', bbox={'facecolor': color[idx], 'alpha': 0.6, 'pad': 1})
        if 'box_label' in tgt:
            ...
            for idx, (bl, score) in enumerate(zip(tgt['box_label'], tgt['scores'])):
                _string = str(bl) + ':' + str(score.item())[:4]
                bbox_x, bbox_y, bbox_w, bbox_h = boxes[idx]
                # ax.text(bbox_x, bbox_y, _string, color='black', bbox={'facecolor': 'yellow', 'alpha': 1.0, 'pad': 1})
                ax.text(bbox_x, bbox_y, _string, color='black', bbox={'facecolor': color[idx], 'alpha': 0.6, 'pad': 1})
        if 'caption' in tgt:
